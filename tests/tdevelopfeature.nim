...
suite "develop feature":
  ...
  test "can develop from dir with srcDir":
    cd &"develop/{pkgSrcDirTestName}":
      ...
      check lines.inLines(pkgSetupInDevModeMsg(pkgSrcDirTestName, getCurrentDir()))
  ...
  test "can develop with dependencies":
    cdCleanDir installDir:
      usePackageListFile &"../develop/{pkgListFileName}":
        let (output, exitCode) = execNimble("develop", "--with-dependencies", pkgBName)
        check exitCode == QuitSuccess
        var lines = output.processOutput
        check lines.inLinesOrdered(
          pkgSetupInDevModeMsg(pkgAName, installDir / pkgAName)
  ...
  test "can develop list of packages":
    cdCleanDir installDir:
      usePackageListFile &"../develop/{pkgListFileName}":
        let (output, exitCode) = execNimble("develop", pkgAName, pkgBName)
        check exitCode == QuitSuccess
        var lines = output.processOutput
        check lines.inLinesOrdered(
          pkgSetupInDevModeMsg(pkgAName, installDir / pkgAName)
  ...
  test "can develop global":
    ...
    usePackageListFile &"develop/{pkgListFileName}":
      ...
      cd dependencyPath:
        ...
        var lines = output.processOutput
        let linkFilePath =
          installDir / "links" / depName.getLinkFileDir / depName.getLinkFileName
        check lines.inLinesOrdered(pkgLinkFileSavedMsg(linkFilePath))
        check lines.inLinesOrdered(pkgSetupInDevModeMsg(depName, dependencyPath))
  ...
  test "cannot remove package with develop reverse dependency":
    cdCleanDir installDir:
      usePackageListFile &"../develop/{pkgListFileName}":
        ...
        var lines = output.processOutput
        check lines.inLinesOrdered(
          cannotUninstallPkgMsg(pkgAName, newVersion("0.2.0"), @[installDir / pkgBName])
        )
  ...
  test "can develop hybrid":
    cd &"develop/{pkgHybridName}":
      ...
      check lines.inLinesOrdered(pkgSetupInDevModeMsg(pkgHybridName, getCurrentDir()))

  test "can specify different absolute clone dir":
    let otherDir = installDir / "./some/other/dir"
    cleanDir otherDir
    let (output, exitCode) = execNimble("develop", &"-p:{otherDir}", pkgAUrl)
    check exitCode == QuitSuccess
    check output.processOutput.inLines(
  ...
  test "do not allow multiple path options":
    ...
    let (output, exitCode) =
      execNimble("develop", &"-p:{developDir}", &"-p:{anotherDevelopDir}", pkgAUrl)
    check exitCode == QuitFailure
  ...
  test "add downloaded package to the develop file":
    ...
    cd "develop/dependency":
      usePackageListFile &"../{pkgListFileName}":
        cleanFile developFileName
        let
          (output, exitCode) = execNimble("develop", &"-p:{installDir}", pkgAName)
        ...
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(&"{pkgAName}@0.6.0", pkgAAbsPath, developFileName)
        )
  ...
  test "can add not a dependency downloaded package to the develop file":
    ...
    cd "develop/dependency":
      usePackageListFile &"../{pkgListFileName}":
        ...
        let
          (output, exitCode) =
            execNimble("develop", &"-p:{installDir}", pkgAName, pkgBName)
          pkgAAbsPath = installDir / pkgAName.toLower
        ...
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(&"{pkgBName}@0.2.0", pkgBAbsPath, developFileName)
        )
  ...
  test "add package to develop file":
    ...
    cd dependentPkgPath:
      usePackageListFile &"../{pkgListFileName}":
        ...
        check output.processOutput.inLines(
          pkgAddedInDevFileMsg(depNameAndVersion, depPath, developFileName)
        )
  ...
  test "warning on attempt to add the same package twice":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        pkgAlreadyInDevFileMsg(depNameAndVersion, depPath, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "can add not a dependency to develop file":
    cd dependentPkgPath:
      ...
      let lines = output.processOutput
      check lines.inLines(
        pkgAddedInDevFileMsg("srcdirtest@1.0", srcDirTestPath, developFileName)
      )
  ...
  test "cannot add two packages with the same name to develop file":
    cd dependentPkgPath:
      ...
      check exitCode == QuitFailure
      check output.processOutput.inLines(
        pkgAlreadyPresentAtDifferentPathMsg(depName, depPath, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "found two packages with the same name in the develop file":
    cd dependentPkgPath:
      cleanFile developFileName
      const developFileContent = developFile(@[], @[depPath, dep2Path])
      writeFile(developFileName, developFileContent)
      ...
      check lines.inLinesOrdered(
        pkgFoundMoreThanOnceMsg(
          depName,
          [
            (depPath.absolutePath.Path, developFilePath.Path),
            (dep2Path.absolutePath.Path, developFilePath.Path),
          ].toHashSet,
        )
      )
      check lines.inLinesOrdered(validationFailedMsg)
  ...
  test "remove package from develop file by path":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        pkgRemovedFromDevFileMsg(depNameAndVersion, depPath, developFileName)
      )
  ...
  test "warning on attempt to remove not existing package path":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        pkgPathNotInDevFileMsg(dep2Path, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "warning on attempt to remove not existing package name":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        pkgNameNotInDevFileMsg(notExistingPkgName, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "include develop file":
    ...
    cd dependentPkgPath:
      usePackageListFile &"../{pkgListFileName}":
        cleanFiles developFileName, includeFileName, dependentPkgName.addFileExt(ExeExt)
        ...
        check developFileName.fileExists
        check output.processOutput.inLines(
          inclInDevFileMsg(includeFileName, developFileName)
        )
  ...
  test "warning on attempt to include already included develop file":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        alreadyInclInDevFileMsg(includeFileName, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "can include file pointing to the same package":
    ...
    cd dependentPkgPath:
      usePackageListFile &"../{pkgListFileName}":
        ...
        const expectedFileContent = developFile(@[includeFileName], @[depPath])
  ...
  test "cannot include conflicting develop file":
    cd dependentPkgPath:
      ...
      check lines.inLinesOrdered(
        pkgFoundMoreThanOnceMsg(
          depName,
          [(depPath.Path, developFileName.Path), (dep2Path.Path, includeFileName.Path)].toHashSet,
        )
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "exclude develop file":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        exclFromDevFileMsg(includeFileName, developFileName)
      )
      check parseFile(developFileName) == parseJson(emptyDevelopFileContent)
  ...
  test "warning on attempt to exclude not included develop file":
    cd dependentPkgPath:
      ...
      check exitCode == QuitSuccess
      check output.processOutput.inLines(
        notInclInDevFileMsg((&"../{includeFileName}").normalizedPath, developFileName)
      )
      check parseFile(developFileName) == parseJson(developFileContent)
  ...
  test "relative paths in the develop file and absolute from the command line":
    cd dependentPkgPath:
      cleanFiles developFileName, includeFileName
      const developFileContent = developFile(@[includeFileName], @[depPath])
      writeFile(developFileName, developFileContent)
      const includeFileContent = developFile(@[], @[depPath])
      writeFile(includeFileName, includeFileContent)
      ...
      let
        ...
        (output, exitCode) = execNimble(
          "develop", &"-e:{includeFileAbsolutePath}", &"-r:{dependencyPkgAbsolutePath}"
        )

      check exitCode == QuitSuccess
      var lines = output.processOutput
      check lines.inLinesOrdered(
        exclFromDevFileMsg(includeFileAbsolutePath, developFileName)
      )
      check lines.inLinesOrdered(
        pkgRemovedFromDevFileMsg(
  ...
  test "absolute paths in the develop file and relative from the command line":
    cd dependentPkgPath:
      let
        ...
        developFileContent = developFile(@[includeFileAbsPath], @[dependencyAbsPath])
      ...
      let (output, exitCode) =
        execNimble("develop", &"-e:{includeFileName}", &"-r:{depPath}")

      check exitCode == QuitSuccess
      var lines = output.processOutput
      check lines.inLinesOrdered(exclFromDevFileMsg(includeFileName, developFileName))
      check lines.inLinesOrdered(
        pkgRemovedFromDevFileMsg(depNameAndVersion, depPath, developFileName)
      )
      check parseFile(developFileName) == parseJson(emptyDevelopFileContent)
      ...
      usePackageListFile &"../{pkgListFileName}":
        ...
        block checkSuccessfulInstallAndReverseDependencyAddedToNimbleData:
          let
            ...
            devRevDepPath =
              nimbleData{$ndjkRevDep}{pkgAName}{"0.5.0"}{checksum}{0}{$ndjkRevDepPath}
        ...
        check lines.inLinesOrdered(
          pkgFoundMoreThanOnceMsg(
            ...
            [
              ...
              (pkg32AbsPath.Path, pkg2DevFileAbsPath.Path),
            ].toHashSet,
          )
        )
  ...
  test "relative include paths are followed from the file's directory":
    cd dependentPkgPath:
      ...
      cleanFiles includeFilePath, developFileName, dependentPkgName.addFileExt(ExeExt), "config.nims"
      ...
      echo "****** file ( " & includeFilePath.absolutePath & ")**** "
      echo includeFileContent
      echo "****************"
      let (output, errorCode) = execNimble("run", "-n")
      echo "********* output *********"
      echo output
      echo "**************************"
  ...
  test "do not filter not used included develop dependencies":
    ...
    cd "develop":
      ...
      cd "pkg1":
        ...
        check exitCode == QuitSuccess
        var lines = output.processOutput
        check lines.inLinesOrdered(pkgDepsAlreadySatisfiedMsg(("pkg2", anyVersion)))
        check lines.inLinesOrdered(pkgDepsAlreadySatisfiedMsg(("pkg3", anyVersion)))
  ...
  test "version clash with not used included develop dependencies":
    ...
    cd "develop":
      const
        ...
        pkg1DevFileContent = developFile(@[&"../{freeDevFile1Name}"], @["../pkg2.2"])
      ...
      cleanFiles pkg1DevFilePath, pkg2DevFilePath, freeDevFile1Name, freeDevFile2Name
      writeFile(pkg1DevFilePath, pkg1DevFileContent)
      writeFile(pkg2DevFilePath, pkg2DevFileContent)
      ...
      cd "pkg1":
        ...
        check lines.inLinesOrdered(
          pkgFoundMoreThanOnceMsg(
            "pkg3",
            [(pkg3Path, freeDevFile1Path), (pkg32Path, freeDevFile2Path)].toHashSet,
          )
        )
  ...
  test "version clash with used included develop dependencies":
    ...
    cd "develop":
      const
        ...
        freeDevFile1Name = "develop1.json"
        freeDevFile2Name = "develop2.json"
        pkg1DevFileContent = developFile(@[&"../{freeDevFile1Name}"], @["../pkg2"])
        pkg2DevFileContent = developFile(@[&"../{freeDevFile2Name}"], @[])
        freeDevFile1Content = developFile(@[], @["./pkg3"])
        freeDevFile2Content = developFile(@[], @["./pkg3.2"])
      ...
      cd "pkg1":
        ...
        var lines = output.processOutput
        check lines.inLinesOrdered(
          failedToLoadFileMsg(getCurrentDir() / developFileName)
        )
  ...
  test "create an empty develop file in some dir":
    ...
    let (output, errorCode) =
      execNimble("--debug", "develop", &"--develop-file:{filePath}")
    check errorCode == QuitSuccess
  ...
  test "can manipulate a free develop file":
    ...
    cd dependentPkgPath:
      usePackageListFile &"../{pkgListFileName}":
        ...
        check lines.inLinesOrdered(inclInDevFileMsg(includeFileName, developFileName))
  ...
  test "add develop --with-dependencies packages to free develop file":
    cdCleanDir installDir:
      ...
      usePackageListFile &"../develop/{pkgListFileName}":
        ...
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(&"{pkgAName}@0.2.0", pkgAPath, developFile)
        )
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(&"{pkgBName}@0.2.0", pkgBPath, developFile)
        )
    ...
    cd dependentPkgPath:
      usePackageListFile &"../{pkgListFileName}":
        ...
        let (output, errorCode) = execNimble(
          ...
          &"-i:{invalidInclFilePath}",
        ) # fail

        check errorCode == QuitFailure
        var lines = output.processOutput
        check lines.inLinesOrdered(
          pkgSetupInDevModeMsg(pkgAName, installDir / pkgAName.toLower)
        )
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(depNameAndVersion, depPath, developFileName)
        )
        ...
        check lines.inLinesOrdered(
          pkgFoundMoreThanOnceMsg(
            ...
            [
              ...
              (dep2Path.Path, includeFileName.Path),
            ].toHashSet,
          )
        )
        check lines.inLinesOrdered(
        ...
        check lines.inLinesOrdered(
          pkgAddedInDevFileMsg(depNameAndVersion, dep2Path, developFileName)
        )
        check lines.inLinesOrdered(inclInDevFileMsg(includeFileName, developFileName))
        check lines.inLinesOrdered(failedToLoadFileMsg(invalidInclFilePath))
        let expectedDevelopFileContent =
          developFile(@[includeFileName], @[dep2Path, installDir / pkgAName.toLower])
