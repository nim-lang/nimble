...
import parseutils, os, osproc, strutils, tables, pegs, uri, strformat,
       json, sequtils, chronos/asyncproc, chronos,
       chronos/apps/http/httpclient, stew/[base10, byteutils]
...
proc updateSubmodules(downloadDir: string) {.async.} =
...
proc doCheckout(meth: DownloadMethod, downloadDir, branch: string):
  ...
  of DownloadMethod.git:
    ...
    await downloadDir.updateSubmodules
...
proc cloneSpecificRevision(downloadMethod: DownloadMethod,
  ...
  of DownloadMethod.git:
    ...
    discard await tryDoCmdExAsync("git", @["-C", downloadDir, "remote", "add", "origin", url])
    discard await tryDoCmdExAsync("git", @["-C", downloadDir, "fetch", "--depth", "1", "origin", $vcsRevision])
    discard await tryDoCmdExAsync("git", @["-C", downloadDir, "reset", "--hard", "FETCH_HEAD"])
    await downloadDir.updateSubmodules
  of DownloadMethod.hg:
...
proc getUrlContent(url: string): Future[string] {.async.} =
  ...
  let
    ...
    adress = session.getAddress(url).get
    req = HttpClientRequestRef.get(session, adress)
    res = await req.send()
    status = res.status

  if status < 200 or status >= 300: # status is not 2xx (success)
      raise nimbleError("Cannot get url content for '$1', returned status $2" % [url, $status])
...
proc downloadPkg*(url: string, verRange: VersionRange,
  ...
  var downloadsInProgress {.global.}: Table[tuple[modUrl: string, downloadDir: string],
                                            Future[tuple[version: Version, vcsRevision: Sha1HashRef]]] =
     initTable[tuple[modUrl: string, downloadDir: string], Future[tuple[version: Version, vcsRevision: Sha1HashRef]]]()

  let key = (modUrl, downloadDir)
  if not downloadsInProgress.hasKey key:
    downloadsInProgress[key] =
       doDownload(modUrl, downloadDir, verRange, downMethod, options, vcsRevision)

  (result.version, result.vcsRevision) = await downloadsInProgress[key]
