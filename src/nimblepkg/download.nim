...
import
  parseutils, os, osproc, strutils, tables, uri, strformat, httpclient, json, sequtils,
  urls

from algorithm import SortOrder, sorted

import
  packageinfotypes, packageparser, version, tools, common, options, cli, sha1hashes,
  vcstools, displaymessages, packageinfo, config, declarativeparser, packagemetadatafile

type DownloadPkgResult* = tuple[dir: string, version: Version, vcsRevision: Sha1Hash]

proc updateSubmodules(dir: string) =
  discard
    tryDoCmdEx(&"git -C {dir.quoteShell} submodule update --init --recursive --depth 1")
...
proc gitFetchTags*(repoDir: string, downloadMethod: DownloadMethod, options: Options) =
  ...
  of DownloadMethod.git:
    let submoduleFlag =
      if not options.ignoreSubmodules: " --recurse-submodules" else: ""
...
proc getTagsListRemote*(url: string, meth: DownloadMethod): seq[string] =
  ...
  of DownloadMethod.git:
    ...
    if exitCode != QuitSuccess:
      raise nimbleError(
        "Unable to query remote tags for " & url & " . Git returned: " & output
      )
    for i in output.splitLines():
...
proc getVersionList*(tags: seq[string]): OrderedTable[Version, string] =
  ...
  let taggedVers: seq[tuple[ver: Version, tag: string]] = tags
    .filterIt(it != "")
    .map(
...
proc downloadTarball(url: string, options: Options): bool =
  ...
  options.enableTarballs and not options.forceFullClone and url.isGitHubRepo and hasTar() and
...
proc removeTrailingGitString*(url: string): string =
  ...
  if url.len > 4 and url.endsWith(".git"):
    url[0 ..^ 5]
  else:
    url
...
proc getFullRevisionFromGitHubApi(url, version: string): Sha1Hash =
  ...
  try:
    ...
    raise nimbleError(
      &"Cannot get revision for version \"{version}\" " & &"of package at \"{url}\".",
      details = error,
    )
...
proc doDownloadTarball(
    url, downloadDir, version: string, queryRevision: bool
): Sha1Hash =
...
proc doDownload(
  ...
  if vcsRevision != notSetSha1Hash:
    ...
    if verRange.spe == getHeadName(downMethod): # Grab HEAD.
      if downloadTarball(url, options):
      ...
      if downloadTarball(url, options):
        result.vcsRevision = doDownloadTarball(url, downloadDir, specialVersion, true)
      else:
        # Grab the full repo.
    ...
    of DownloadMethod.git:
      ...
      if versions.len > 0:
        getLatestByTag:
          if downloadTarball(url, options):
            let versionToDownload = if latest.tag.len > 0: latest.tag else: "HEAD"
            result.vcsRevision =
              doDownloadTarball(url, downloadDir, versionToDownload, true)
          else:
            display(
              "Cloning",
              "latest tagged version: " & latest.tag,
        ...
        display(
          "Warning:",
          "Package from '" & url & "' has no tagged releases, downloading HEAD instead.",
          Warning,
    ...
    of DownloadMethod.hg:
      ...
      if versions.len > 0:
        getLatestByTag:
          display(
            "Switching",
            "to latest tagged version: " & latest.tag,
            priority = MediumPriority,
          )
...
proc downloadPkg*(
  ...
  let (downloadDir, pkgDir) =
    downloadPkgDir(url, verRange, subdir, options, vcsRevision, downloadPath)
  result.dir = pkgDir
  ...
  let downloadMethod =
    if downloadTarball(modUrl, options):
      "http"
    else:
      $downMethod
  ...
  if subdir.len > 0:
    display(
      "Downloading",
      "$1 using $2 (subdir is '$3')" % [modUrl, downloadMethod, subdir],
      priority = HighPriority,
    )
  else:
    display(
      "Downloading", "$1 using $2" % [modUrl, downloadMethod], priority = HighPriority
    )

  (result.version, result.vcsRevision) =
  ...
  if validateRange and verRange.kind notin {verSpecial, verAny} or not options.isLegacy:
    ...
    pkginfo =
      if options.satResult.pass in {satNimSelection, satFallbackToVmParser}:
        #TODO later when in vnext we should just use this code path and fallback inside the toRequires if we can
    ...
    if pkginfo.basicInfo.version notin verRange:
      raise nimbleError(
        "Downloaded package's version does not satisfy requested version " &
          "range: wanted $1 got $2." % [$verRange, $pkginfo.basicInfo.version]
      )
...
proc echoPackageVersions*(pkg: Package) =
  ...
  of DownloadMethod.hg:
    displayInfoLine(
      "  versions:    ",
      "(Remote tag retrieval not supported by " & $pkg.downloadMethod & ")",
    )
...
proc refresh*(options: Options) =
  ...
  let parameter =
    if options.action.typ == actionRefresh: options.action.optionalURL else: ""
...
proc getDownloadInfo*(
    pv: PkgTuple, options: Options, doPrompt: bool, ignorePackageCache = false
  ...
  if getPackage(pv.name, options, pkg, ignorePackageCache):
    ...
    if doPrompt and not options.offline and
        options.prompt(
          pv.name & " not found in any local packages.json, " &
...
when isMainModule:
  ...
  suite "version sorting":
    test "pre-release versions":
      let data =
        @[
          "v9.0.0-taeyeon", "v9.0.1-jessica", "v9.2.0-sunny", "v9.4.0-tiffany",
          "v9.4.2-hyoyeon",
        ]
    ...
    test "release versions":
      let data = @["v0.1.0", "v0.1.1", "v0.2.0", "0.4.0", "v0.4.2"]
      let expected = toOrderedTable[Version, string](
        {
  ...
  suite "getDevelopDownloadDir":
    ...
    let dummyOptionsWithAbsolutePath =
      Options(action: Action(typ: actionDevelop, path: "/some/dir/"))
    let dummyOptionsWithRelativePath =
      Options(action: Action(typ: actionDevelop, path: "some/dir/"))

    test "without subdir and without path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/packagea/", "", dummyOptionsWithoutPath
      ) == getCurrentDir() / "packagea"

    test "without subdir and with absolute path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/packagea", "", dummyOptionsWithAbsolutePath
      ) == "/some/dir/packagea".normalizedPath

    test "without subdir and with relative path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/packagea/", "", dummyOptionsWithRelativePath
      ) == getCurrentDir() / "some/dir/packagea"

    test "with subdir and without path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/multi", "beta", dummyOptionsWithoutPath
      ) == getCurrentDir() / "beta"

    test "with subdir and with absolute path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/multi/", "alpha/", dummyOptionsWithAbsolutePath
      ) == "/some/dir/alpha".normalizedPath

    test "with subdir and with relative path":
      check getDevelopDownloadDir(
        "https://github.com/nimble-test/multi", "alpha/", dummyOptionsWithRelativePath
      ) == getCurrentDir() / "some/dir/alpha"
